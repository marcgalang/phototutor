...
function fphotos(criteria){	
	num=0
	console.log("running fphotos");
	photopool=[]
	$("#start").hide();
	//constructing PHOTO SEARCH sig and API url
	
	sig=MD5(fs+"api_key"+fk+"auth_token"+token+"formatjson"+"methodflickr.interestingness.getList"+"nojsoncallback1page"+page+"per_page50");
	var APIstr="https://api.flickr.com/services/rest/?method=flickr.interestingness.getList&api_key="+fk+"&page="+page+"&per_page=50&format=json&nojsoncallback=1&auth_token="+token+"&api_sig="+sig;
	...
	$.getJSON(APIstr, function(data) {		
		$.each(data.photos.photo,function(i,mypic){
			...
			$.getJSON(APIstr,function(exifdata){
				console.log(j);
				console.log(exifdata.code);
				j+=1;
				if(exifdata.code!=2){
					photopool.push(exifdata);
				}	
				console.log(photopool.length+" in pool");
				if (j==data.photos.photo.length){
					reviewPhotopool();	
				};	
			});
		});
...
function showPwX (){
	...
	$.each(picswX,function(i,mypic){
		...
		$("#pics").append("<div id='pic"+i+"' style='float:left;' class='image'><img id='img"+i+"' src="+myurl+" alt='flicker pic #"+i+"' class='img-responsive'><div id='question"+i+"' class='question "+i+"'>"+question+"</div><div id='hider"+i+"' class='hider'><p>Move the cursor here to examine the photo better.</p></div><div id='response"+i+"' class='response container "+i+"'><form role='form'><label class='radio-inline'><input type='radio' name='optradio'>"+response1+"</label><label class='radio-inline'><input type='radio' name='optradio'>"+response2+"</label><label class='radio-inline'><input type='radio' name='optradio'>Pass. It\'s too hard to tell.</label></form></div><div id='exif"+i+"' class='exif "+i+"'>"+"<ul><li>ISO:"+mypic.ISO+"</li><li>Exposure:"+mypic.ExposureTime+"</li><li>f/number:"+mypic.FNumber+"</li><li>Flash:"+mypic.Flash+"</li>"+"</ul></div><div id='feedback"+i+"' class='feedback "+i+"'>"+feedback+"</div><div id='selfscore"+i+"' class='selfscore container "+i+"'><p>how would you rate your answer?</p><form role='form'><label class='radio-inline'><input type='radio' name='optradio' id='plusone"+i+"'>I was right.</label><label class='radio-inline'><input type='radio' name='optradio'>Not quite right.</label><label class='radio-inline'><input type='radio' name='optradio' id='pass"+i+"'>Pass. It was too hard to tell.</label></form></div></div>");
